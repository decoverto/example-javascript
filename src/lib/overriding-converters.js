...
let OverrideSingleConverter = _decorate([model()], function (_initialize) {
  ...
  return {
    F: OverrideSingleConverter,
    d: [{
      kind: "field",
      decorators: [property(() => Date, {
        toPlain: date => date == null ? date : date.getTime()
      })],
      key: "date",
      value: void 0
    }]
...
console.log('The overridden converter is used for toPlain', singleOverrideHandler.instanceToPlain(singleConverterResult));
/*
 * You could also set both converters.
 */

let OverrideConverters = _decorate([model()], function (_initialize2) {
...
console.log('Both converters overwritten, to plain:', overrideConvertersHandler.instanceToPlain(overrideConvertersResult));